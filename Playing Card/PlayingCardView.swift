...
//  Playing Card
//
//  Created by Michael Parton on 15/8/18.
...
class PlayingCardView: UIView {
    ...
    @objc func adjustCardScale(gesture recognizer: UIPinchGestureRecognizer) {
    ...
    private var cornerText:NSAttributedString {
    ...
    private func configureCornerLabel(_ label: UILabel) {
        label.attributedText = cornerText
    ...
    private func drawPips() {
        ...
        func createPipString(thatFits pipRect: CGRect) -> NSAttributedString {
            ...
            let probablyOkayPipString = centeredAttributedString(string: suit, size: probablyOKPipStringFontSize)
            
            if probablyOkayPipString.size().width > pipRect.size.width / maxHorizontalPipCount {
                return centeredAttributedString(string: suit, size: probablyOKPipStringFontSize / (probablyOkayPipString.size().width / (pipRect.size.width / maxHorizontalPipCount)))
            } else {
        ...
        if pipsPerRowForRank.indices.contains(rank) {
            ...
            var pipRect = bounds.insetBy(dx: cornerOffset, dy: cornerOffset).insetBy(dx: cornerText.size().width, dy: cornerText.size().height / 2)
